...
def main(sdr_type: Text = None) -> NoReturn:
    storage = dict()
    if sdr_type is None:
        return None
    proc = Popen(
    ...
    if proc.stdout and sdr_type == "Fan":
        ...
        for i in result.decode().split("\n"):
            if len(i) > 0:
                for idx, val in enumerate(i.split("|")):
                    if idx == 0:
                        _match = search("(^PS\d|^PSU\d)", val)
                        if _match:
                            if (
                                val[_match.span()[1] - 1].isdigit
                                and psu_count == -1
                            ):
                ...
                for idx, val in enumerate(unit):
                    if idx == 4 and isinstance(unit[val], dict):
                        if unit[val].get("power_unit_group"):
                            storage[str(unit[val]["power_unit_group"])] = unit[
                                val
                            ]
                        else:
                            _psu_counter += 1
        ...
        if args.type[0] not in ["Fan", "Power Supply"]:
            print(
                json.dumps(
                    {
                        "State": (
                            "Possible values is only ['Fan', 'Power Supply']"
                        )
                    }
                )
            )
